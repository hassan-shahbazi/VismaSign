...
protocol APIRequestHeader {
    ...
    init(_ request: ServiceRequest, _ headerSet: HeaderSet)
}
...
public struct APIRequestHeaderImpl<T: APIServiceRequest>: APIRequestHeader {
    ...
    init(_ request: ServiceRequest, _ headerSet: HeaderSet) {
        ...
        switch headerSet {
            ...
            case .organization(let secret, let clientID, let date):
                self.headers = commonHeaders(date)
...
enum HeaderSet {
    ...
    case organization(String, String, Date)
    static func organization(_ clientID: String, _ secret: String) -> HeaderSet {
        return .organization(clientID, secret, Date())
    }
